...
app_drv_fifo_write(app_drv_fifo_t *fifo, uint8_t *data, uint16_t *p_write_length)
{
    ...
    //PRINT("fifo_length = %d\n",fifo_length(fifo));
    uint16_t available_count = fifo->size - fifo_length(fifo);
    uint16_t requested_len = (*p_write_length);
    if(available_count < requested_len) {
        PRINT("FIFO full! %d < %d\n", available_count, requested_len);
        return APP_DRV_FIFO_RESULT_LENGTH_ERROR;
    }
    uint16_t index = 0;
...
app_drv_fifo_write_from_same_addr(app_drv_fifo_t *fifo, uint8_t *data, uint16_t write_length)
{
    ...
    uint16_t available_count = fifo->size_mask - fifo_length(fifo) + 1;
    uint16_t requested_len = (write_length);
    uint16_t index = 0;
    uint16_t write_size = MIN(requested_len, available_count);
...
app_drv_fifo_read(app_drv_fifo_t *fifo, uint8_t *data, uint16_t *p_read_length)
{
    ...
    uint16_t byte_count = fifo_length(fifo);
    uint16_t requested_len = (*p_read_length);
    uint32_t index = 0;
...
app_drv_fifo_read_to_same_addr(app_drv_fifo_t *fifo, uint8_t *data, uint16_t read_length)
{
    ...
    uint16_t byte_count = fifo_length(fifo);
    uint16_t requested_len = (read_length);
    uint32_t index = 0;
    uint32_t read_size = MIN(requested_len, byte_count);
